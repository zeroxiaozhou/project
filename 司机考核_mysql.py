...
db = pymysql.connect(host='HgDataBase', port=4408, user='root', password='M25xx%ycl', database='sophon')
def delete(sql):
    cursor = db.cursor()
    cursor.execute(sql)
    db.commit()
...
sql = f"""delete from driver_assessment_data where date_time='{start_Time}'"""
delete(sql)
time.sleep(2)
engine = create_engine('mysql+pymysql://root:M25xx%ycl@HgDataBase:4408/sophon?utf8mb4', echo=False)
with engine.connect() as connection:
    info = connection.execute(text("SELECT field_cn, field_en FROM `edsac`.`fields_info` WHERE table_name='driver_assessment_data'"))
        ...
        try:
            data = {
                ...
                'exportParams': {
                    ...
                    'tenant_id': {'matchValue': account, 'value': [dit[account]['eid']]}},
...
data_list = ['司机ID', '司机姓名', '司机手机号', '司机类型', '激活时间', '状态',
             ...
             '订单总金额', '投诉订单量',
             '行程费退款金额', '远程调度费退款金额', '附加费退款金额', '行程费',
             '远程调度费', '附加费', '司机抽佣行程费', '司机订单收入', '奖励金额-首单奖', '奖励金额-冲单奖',
             '总奖励金额', '早高峰应答量', '早高峰完单量',
...
for account in dit:
    ...
    # audit_url = 'https://admin.yueyuechuxing.cn/admin/v2/common/export/server/getExportRecordsV2'
...
# 更新近30天司机考核
# to_day30ago_kpi()
# sql2 = f"""delete from driver_assessment_data where finish_order_sum='0.0'"""
# delete(sql2)
